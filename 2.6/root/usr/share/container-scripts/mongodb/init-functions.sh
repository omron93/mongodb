...
function mongo_create_admin() {
  ...
  # Set admin password
  set +e
  mongo admin ${1:-} --eval "db.createUser({user: 'admin', pwd: '${MONGODB_ADMIN_PASSWORD}', roles: ['dbAdminAnyDatabase', 'userAdminAnyDatabase' , 'readWriteAnyDatabase','clusterAdmin' ]});"
  result=$?
  set -e
...
function mongo_create_user() {
  ...
  # Crate database user
  set +e
  mongo admin ${1:-} --eval "db.getSiblingDB('${MONGODB_DATABASE}').createUser({user: '${MONGODB_USER}', pwd: '${MONGODB_PASSWORD}', roles: [ 'readWrite' ]});"
  result=$?
  set -e
